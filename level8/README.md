Funtion takes an input will displaying `(nil) ,(nil)`
```
   ...
   <0> - <14>: initialize stack, align 16, allocate 160 bytes for local var
   0x08048574 <+16>:	nop
   ...
   <16> - <45>: initialize args and call printf
   0x08048596 <+50>:	mov    0x8049a80,%eax (eax = stdin)
   0x0804859b <+55>:	mov    %eax,0x8(%esp) 
   0x0804859f <+59>:	movl   $0x80,0x4(%esp) - 128 bytes allocated for fgets buffer
   0x080485a7 <+67>:	lea    0x20(%esp),%eax - pointer for fgets()
   0x080485ab <+71>:	mov    %eax,(%esp) 
   0x080485ae <+74>:	call   0x8048440 <fgets@plt>
   0x080485b3 <+79>:	test   %eax,%eax - cmp eax, 0
   0x080485b5 <+81>:	je     0x804872c <main+456> - jump if null returned
   ...
   <87> - <107>: ~~ strncmp(buffer, "auth ", 5)
   0x080485d1 <+109>:	seta   %dl - 1 if CF = 0 && ZF = 0
   0x080485d4 <+112>:	setb   %al - 1 if CF = 1
   0x080485d7 <+115>:	mov    %edx,%ecx
   0x080485d9 <+117>:	sub    %al,%cl
   0x080485db <+119>:	mov    %ecx,%eax
   0x080485dd <+121>:	movsbl %al,%eax - copies value al with sign ext
   0x080485e0 <+124>:	test   %eax,%eax
   0x080485e2 <+126>:	jne    0x8048642 <main+222> if 0, jup at 222
   <87> - <126>: ~~ if (strncmp(buffer, "auth ", 5) != 0)
   0x080485e4 <+128>:	movl   $0x4,(%esp)
   0x080485eb <+135>:	call   0x8048470 <malloc@plt> - malloc 4 bytes
   0x080485f0 <+140>:	mov    %eax,0x8049aac
   0x080485f5 <+145>:	mov    0x8049aac,%eax
   <128> - <145>: global var "auth " = malloc(4)
   0x080485fa <+150>:	movl   $0x0,(%eax) - auth[0] = 0
   0x08048600 <+156>:	lea    0x20(%esp),%eax - eax = buffer
   0x08048604 <+160>:	add    $0x5,%eax - buffer + 5
   0x08048607 <+163>:	movl   $0xffffffff,0x1c(%esp) - set to -1
   0x0804860f <+171>:	mov    %eax,%edx
   0x08048611 <+173>:	mov    $0x0,%eax
   0x08048616 <+178>:	mov    0x1c(%esp),%ecx
   <171> - <178>: save eax in edx, set eax to 0, store esp +0x1c in ecx
   0x0804861a <+182>:	mov    %edx,%edi - edi points to buffer + 5
   0x0804861c <+184>:	repnz scas %es:(%edi),%al
   0x0804861e <+186>:	mov    %ecx,%eax
   0x08048620 <+188>:	not    %eax
   <182> - <188>:  ~~ strlen(buffer + 5)
   0x08048622 <+190>:	sub    $0x1,%eax
   0x08048625 <+193>:	cmp    $0x1e,%eax
   0x08048628 <+196>:	ja     0x8048642 <main+222>
   <190> - <196>: ~~ if (strlen(buffer + 5) > 30)
   ...
   <222> - <242>: ~~ strncmp(buffer, "reset", 5)
   ...
   <222> - <261>: ~~ if(strncmp(buffer, "reset", 5) != 0)
   ...
   <276> - <296>: ~~ strncmp(buffer, "service", 6)
   ...
   <332> - <357>: ~~ strncmp(buffer, "login", 5)
   ...
   <332> - <376>: ~~ if(strncmp(buffer, "login", 5) != 0)
   0x080486e2 <+382>:	mov    0x8049aac,%eax
   0x080486e7 <+387>:	mov    0x20(%eax),%eax
   0x080486ea <+390>:	test   %eax,%eax
   0x080486ec <+392>:	je     0x80486ff <main+411>
   <382> - <392>: ~~ if(auth[32] != 0)
   0x080486ee <+394>:	movl   $0x8048833,(%esp)
   0x080486f5 <+401>:	call   0x8048480 <system@plt>
   <394> - <401>: system("/bin/sh")
   0x080486fa <+406>:	jmp    0x8048574 <main+16> - je begining of loop
   ...
   <411> - <446>: set args for fwrite("Password:\n", 1, 1à, stdout)
   ...
   0x08048738 <+468>:	ret
   <451> - <468>: end flow of program main

```

We have learned from the asm that the function will take arguments:
"auth " = - which will malloc 4 bytes in the global variable
	- set auth\[0] = 0
	- cpy contents of buffer + 5 in auth
"reset" = will free auth
"service" = service(strdup(buffer + 7))
"login" = - if(auth\[32] != 0) system("/bin/sh")
	- else fwrite("Password:\n", 10, 1, stdout)
Now we have to fill auth up to 32 bytes so we can get to system() call
```
...
```
In the first method we can see that "auth" and "service" are placed exactly 16 bytes apart, so we could call service 2 times so like this we fill 32 bytes for auth.
In the second method we can just add 16 bytes after service and it will copy the contents in auth
```
level8@RainFall:~$ ./level8 
(nil), (nil) 
auth 
